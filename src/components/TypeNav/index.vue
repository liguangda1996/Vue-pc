<template>
  ...
  <div class="type-nav">
    <div class="container">
      <h2 class="all" @mouseenter="isSearchShow = true" @mouseleave="isSearchShow = false">全部商品分类</h2>
      <nav class="nav">
        <a href="###">服装城</a>
        <a href="###">美妆馆</a>
        <a href="###">尚品汇超市</a>
...
export default {
  ...
  methods: {
    ...
    goSearch(e) {
      ...
      const { categoryname, categoryid, categorytype } = e.target.dataset; // dataset来获取自定义属性 添加自定义属性，加前缀data-  同时这些属性名都会被转为小写
      // 隐藏分类列表
  ...
  mounted() {
    console.log(this);
    // 在请求之前先判断vuex有没有数据 有数据就调用 减少请求次数
